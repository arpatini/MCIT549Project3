...
int main(int argc, char *argv[]) {
    ...
    cmd.AddValue ("wifiMcs", "HtMcs1 or HtMcs7 for wifi configuration", phyRate);
    cmd.AddValue ("tcpRcvBufBytes", "TCP receive buffer size in bytes", tcpRcvBufBytes);
    cmd.AddValue ("delayValueforRHtoR", "one-way link delay in msec between Remote Host and Router", delayValueBtwnRemoteHostAndRouter);
    cmd.AddValue ("delayValueforWifi", "link delay in msec between router and Wifi AP", delayValueforWifi);
    ...
    if (ueNodes.GetN() > 20 || ueNodes.GetN() < 1) {
        printf("INPUT ERROR\n");
        exit(1);
    }
    for (uint16_t i = 1; i <= ueNodes.GetN(); i++) {
        tmpX = (ueXmax - ueXmin) * ( (float)rand() / RAND_MAX ) + ueXmin;
        tmpY = (ueYmax - ueYmin) * ( (float)rand() / RAND_MAX ) + ueYmin;
...
//------------------------------------------------------------------------------//
    ...
    p2ph.EnablePcap(prefix_file_name + "01", 0, 1);
    p2ph.EnablePcap(prefix_file_name + "11", 1, 1);
    p2ph.EnablePcap(prefix_file_name + "13", 1, 3);
